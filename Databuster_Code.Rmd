...
>>>>>>> b5f1d3f3f3755de013ea3ec778921140e7c7ab21
...
value_CPEIFL <- forecast(model2, h=5)
print(value_CPEIFL)
...
point_forecasts_CPEIFL <- value_CPEIFL$mean
point_forecasts_values_1 <- as.numeric(point_forecasts_CPEIFL) 
...
old_gdp_data <- df_select %>%
  ...
  mutate(quarter_year_labels = paste0(format(sasdate, "%Y"), "-Q", 
                                      ceiling(as.numeric(format(sasdate, "%m")) / 3))) %>%
...
set.seed(123)
n_simulations <- 1000
forecast_quantiles <- matrix(NA, nrow = 4, ncol = 9)  # 4 time steps, 9 quantiles (10th to 90th)

for (i in 1:4) {
  simulated_values <- rnorm(n_simulations, mean = forecast_values[i], sd = sd(residuals(final_model)))
  forecast_quantiles[i, ] <- quantile(simulated_values, probs = seq(0.1, 0.9, by = 0.1))
}

# Create the fan chart
plot.new()
plot.window(xlim = c(1, 4), ylim = range(forecast_quantiles), xlab = "Time", ylab = "Forecast Values")
fan(forecast_quantiles, start = 1, frequency = 1, 
    fan.col = colorRampPalette(c("red", "yellow", "green")), 
    ln = NULL, rlab = seq(0.1, 0.9, by = 0.1), rpos = 2)
```
